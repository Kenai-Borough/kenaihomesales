      ...
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.95fr,1fr]">
        ...
        <div className="rounded-[2rem] border border-slate-200/20 bg-white/90 p-8 shadow-2xl backdrop-blur dark:border-white/10 dark:bg-slate-950/85 dark:text-white">
          ...
          <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
            ...
            <label className="flex items-start gap-3 rounded-2xl border border-slate-200 px-4 py-3 text-sm dark:border-white/10"><input type="checkbox" checked={acceptTerms} onChange={function (event) { setAcceptTerms(event.target.checked) }} className="mt-1" /> <span>I agree to the <Link to="/terms" className="text-sky-400 hover:underline">Terms of Service</Link>.</span></label>
            <label className="flex items-start gap-3 rounded-2xl border border-slate-200 px-4 py-3 text-sm dark:border-white/10"><input type="checkbox" checked={acceptPrivacy} onChange={function (event) { setAcceptPrivacy(event.target.checked) }} className="mt-1" /> <span>I agree to the <Link to="/privacy" className="text-sky-400 hover:underline">Privacy Policy</Link>.</span></label>
